<x-layout>
    <x-slot:title>Edit Penawaran Kerja</x-slot:title>

    <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <!-- Header -->
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
            <div>
                <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Edit Penawaran Kerja</h1>
                <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">Ubah data kontrak kerja mitra statistik.</p>
            </div>
            <a href="{{ route('sipetrik.penawaran-kerja.show', $contract->id) }}" 
        ...
        <form action="{{ route('sipetrik.penawaran-kerja.update', $contract->id) }}" method="POST"
              x-data="{ 
                  volume: {{ old('volume', $contract->volume) }}, 
                  harga: {{ old('harga_satuan', $contract->harga_satuan) }}, 
                  total: {{ old('volume', $contract->volume) * old('harga_satuan', $contract->harga_satuan) }}, 
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 relative z-30">
                ...
                <div class="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div x-data='searchableSelect({ 
                            ...
                            selected: {{ old('team_id', $contract->team_id) }},
                    ...
                    <div x-data='searchableSelect({ 
                            ...
                            selected: {{ old('mitra_id', $contract->mitra_id) }},
                ...
                <div class="p-6">
                    <div class="grid grid-cols-1 gap-6">
                        <div x-data='searchableSelect({ 
                                ...
                                selected: {{ old('activity_id', $contract->activity_id) }},
                        ...
                        <div>
                            ...
                            <textarea name="uraian_tugas" rows="3" class="w-full border-gray-300 dark:border-gray-600 rounded-lg dark:bg-gray-700 dark:text-white px-4 py-2.5 focus:ring-orange-500 focus:border-orange-500 shadow-sm transition" placeholder="Deskripsi singkat tugas yang akan dilakukan...">{{ old('uraian_tugas', $contract->uraian_tugas) }}</textarea>
                        </div>
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <input type="date" name="tanggal_mulai" value="{{ old('tanggal_mulai', $contract->tanggal_mulai) }}" required class="w-full border-gray-300 dark:border-gray-600 rounded-lg dark:bg-gray-700 dark:text-white px-4 py-2.5 focus:ring-orange-500 focus:border-orange-500 shadow-sm transition">
                            </div>
                            <div>
                                ...
                                <input type="date" name="tanggal_selesai" value="{{ old('tanggal_selesai', $contract->tanggal_selesai) }}" required class="w-full border-gray-300 dark:border-gray-600 rounded-lg dark:bg-gray-700 dark:text-white px-4 py-2.5 focus:ring-orange-500 focus:border-orange-500 shadow-sm transition">
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                        ...
                        <div>
                            ...
                            <select name="satuan" required class="w-full border-gray-300 dark:border-gray-600 rounded-lg dark:bg-gray-700 dark:text-white px-4 py-2.5 focus:ring-orange-500 focus:border-orange-500 shadow-sm transition">
                                <option value="Dokumen" {{ old('satuan', $contract->satuan) == 'Dokumen' ? 'selected' : '' }}>Dokumen</option>
                                <option value="BS" {{ old('satuan', $contract->satuan) == 'BS' ? 'selected' : '' }}>Blok Sensus</option>
                                <option value="Ruta" {{ old('satuan', $contract->satuan) == 'Ruta' ? 'selected' : '' }}>Rumah Tangga</option>
                                <option value="Segmen" {{ old('satuan', $contract->satuan) == 'Segmen' ? 'selected' : '' }}>Segmen</option>
                                <option value="SLS" {{ old('satuan', $contract->satuan) == 'SLS' ? 'selected' : '' }}>SLS</option>
                                <option value="O-B" {{ old('satuan', $contract->satuan) == 'O-B' ? 'selected' : '' }}>Orang-Bulan</option>
                                <option value="Paket" {{ old('satuan', $contract->satuan) == 'Paket' ? 'selected' : '' }}>Paket</option>
                            </select>
                        </div>
                        <div>
            ...
            <div class="flex justify-end gap-4 pt-4">
                <a href="{{ route('sipetrik.penawaran-kerja.show', $contract->id) }}" class="px-6 py-2.5 text-gray-700 hover:text-gray-900 dark:text-gray-300 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-xl hover:bg-gray-50 dark:hover:bg-gray-700 font-medium transition shadow-sm">Batal</a>
